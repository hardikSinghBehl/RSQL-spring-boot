...
public class IsGreaterThanEqualToConditionTest {
    ...
    @Autowired
    public IsGreaterThanEqualToConditionTest(final WandService wandService, final WandRepository wandRepository) {
    ...
    @Test
    void notGreaterLength_noResult() {
        final String query = "length>=" + (LENGTH + 1);
        final var result = wandService.retreive(RsqlSearchRequestDto.builder().query(query).build()).getResult();
